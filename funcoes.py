...
def cadastrar_livro():
    codigo = input("--------------------------------\nDigite o código do livro: ")  # Solicita ao usuário que digite o código do livro e o armazena na variável 'codigo'
    titulo = input("Digite o título do livro: ")  # Solicita ao usuário que digite o título do livro e o armazena na variável 'titulo'
    autor = input("Digite o autor do livro: ")  # Solicita ao usuário que digite o autor do livro e o armazena na variável 'autor'
    ano_publicacao = input("Digite o ano de publicação do livro: ")  # Solicita ao usuário que digite o ano de publicação do livro e o armazena na variável 'ano_publicacao'
    quant_exemplares = input("Digite a quantidade de exemplares do livro: ")  # Solicita ao usuário que digite a quantidade de exemplares do livro e a armazena na variável 'quant_exemplares'
    ...
    if arquivo_vazio(arquivo):  # Verifica se o arquivo está vazio chamando a função 'arquivo_vazio' com o nome do arquivo como argumento
        with open(arquivo, "w+") as leitura:  # Abre o arquivo especificado em modo de anexo ("a")
...
def cadastrar_usuario():
    ...
    # Solicita o telefone do usuário ao usuário
    ...
    # O código a seguir adiciona o prefixo "+55" e espaços nos dígitos específicos do número de telefone.

    # Utiliza o método replace para remover espaços em branco do número de telefone e, em seguida, formata-o com os espaços corretos.
    # O código [:2] retorna os primeiros dois dígitos do número.
    # O código [2:7] retorna os dígitos do índice 2 até o índice 6.
    # O código [7:] retorna os dígitos a partir do índice 7 até o final do número.
    ...
    if arquivo_vazio(arquivo):
        ...
        with open(arquivo, "a") as leitura:
            # Escreve o cabeçalho no arquivo
            leitura.write("codigo|nome|email|telefone\n")

    # Abre o arquivo no modo de leitura
    with open(arquivo, "r") as arquivo_leitura:
        # Verifica se a linha desejada já existe no arquivo
        if linha_desejada in arquivo_leitura.read():
            print("--------------------------------\nO usuário já está cadastrado.\n--------------------------------")
            return  # Encerra a função se o usuário já estiver cadastrado
...
def verificar_existencia(arquivo, codigo):
    # Função para verificar se uma linha com o código existe no arquivo
    ...
    with open(arquivo, "r") as arquivo_leitura:
        ...
        for linha in arquivo_leitura:
            ...
            partes = linha.split("|")
            # Verifica se o código na primeira parte da linha é igual ao código fornecido
            if partes[0] == codigo:
                return True
    # Retorna False caso não encontre uma linha com o código fornecido
    return False
...
def reservar_livro():
    ...
    if not verificar_existencia("usuarios.txt", codigo_usuario):
    ...
    if not verificar_existencia("livros.txt", codigo_livro):
        print("Livro não encontrado. Cadastre o livro antes de fazer a reserva.")
        return

    # Abre o arquivo "reservas.txt" em modo de leitura
    with open("reservas.txt", "r") as arquivo_leitura:
        # Itera sobre cada linha do arquivo
        for linha in arquivo_leitura:
            # Divide a linha em campos separados pelo caractere "|"
            campos = linha.strip().split("|")
            # Verifica se o primeiro campo (código da reserva) é igual ao código de reserva atual
            if len(campos) >= 1 and campos[0].strip() == codigo_reserva:
                print("--------------------------------\nCódigo de reserva já em uso. Escolha outro código.\n--------------------------------")
                return

    # Abre o arquivo "reservas.txt" novamente em modo de leitura
    with open("reservas.txt", "r") as arquivo_leitura:
        # Itera sobre cada linha do arquivo
        for linha in arquivo_leitura:
            # Divide a linha em campos separados pelo caractere "|"
            campos = linha.strip().split("|")
            # Verifica se o terceiro campo (código do livro) e o segundo campo (código do usuário) são iguais aos códigos atuais
            if len(campos) >= 3 and campos[1].strip() == codigo_usuario and campos[2].strip() == codigo_livro:
                print("--------------------------------\nEste usuário já reservou este livro antes.\n--------------------------------")
                return
